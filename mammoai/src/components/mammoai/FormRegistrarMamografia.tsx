...
interface IFormRegistrarMamografia {
  editar: boolean;
}

export function FormRegistrarMamografia({ editar }: IFormRegistrarMamografia) {
  return (
    <Sheet>
      <SheetTrigger asChild>
        <Button className="flex justify-cente bg-transparent shadow-none text-pink-600 hover:bg-gray-200 hover:border-0">
          {editar ? <SquarePen /> : <Plus />}
          {editar ? "" : "Nuevo"}
        </Button>
      </SheetTrigger>
        ...
        <div className="flex flex-col h-full">
          <SheetHeader>
            <SheetTitle className="flex justify-start items-center text-center gap-2">
              ...
              Registro de {editar ? "" : "nueva"} Mamografía
            </SheetTitle>
          </SheetHeader>
